...
class Script:
    ...
    def _create_machine_domains(self) -> list[Domain]:
        ...
        slot_domains, domain_SurfCoil = self._create_winding_domains()
        # add domains to domain list
        domain_list.extend(slot_domains)
        domain_list.append(domain_SurfCoil)
        ...
        for slot in all_slots:
            stator_physicals[DOMAIN_NON_CONDUCTING].remove(slot)
            stator_physicals[slot_linear_domain].remove(slot)
        # add new slot physicals to stator domains
        new_slots = [slot for domain in slot_domains for slot in domain.physicals]
        stator_physicals[DOMAIN_NON_CONDUCTING].extend(new_slots)
        stator_physicals[slot_linear_domain].extend(new_slots)
    ...
    def _create_winding_domains(self) -> dict[str, Domain]:
        """
        ...
        for (phase, direction), slots in slot_dict.items():
            ...
            # winding direction
            slot_surfaces = [surface for slot in slots for surface in slot.geo_list]
    ...
    def write_pro(self):
        ...
        shutil.copy(
            ...
            join(self.script_path, "Circuit.pro"),
        )
